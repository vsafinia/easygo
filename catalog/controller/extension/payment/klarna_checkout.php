...
class ControllerExtensionPaymentKlarnaCheckout extends Controller {
	...
	public function validation() {
		...
		if ($validate) {
			...
			if ($order_info) {
				...
				if (round($this->currency->format($total, $order_info['currency_code'], $order_info['currency_value'], false) * 100) != $request->order_amount) {
					$this->model_extension_payment_klarna_checkout->log('Klarna Checkout order_amount does not match session order total. Klarna Request: ' . $request->order_amount . '. EasyGo: ' . round($this->currency->format($total, $order_info['currency_code'], $order_info['currency_value'], false) * 100));
		...
		if ($validate) {
			...
			// Update EasyGo order with payment and shipping details
			...
			//If country isn't GB, try to update EasyGo order with correct region/zone
